...
const LegendItem = ({ name, color, onClick, activeLegend, chartType }: LegendItemProps) => {
  ...
  return (
    <li
      className={cx(
        ...
        hasOnValueChange ? "cursor-pointer hover:bg-neutral-100 dark:hover:bg-neutral-800" : "cursor-default",
      )}
      onClick={(e) => {
        e.stopPropagation();
        onClick?.(name, color);
    ...
    >
      ...
      <p
        className={cx(
          ...
          hasOnValueChange && "group-hover:text-neutral-900 dark:group-hover:text-neutral-50",
          activeLegend && activeLegend !== name ? "opacity-40" : "opacity-100",
        )}
      >
        {name}
    ...
    <ol ref={ref} className={cx("relative overflow-hidden", className)} {...other}>
      ...
      >
        {categories.map((category, index) => {
          if (!barCategoryColors.has(category.name) || !lineCategoryColors.has(category.name)) {
            console.warn(`Color not found for ${category.name}`);
          }
          const barColor = barCategoryColors.get(category.name) || "blue";
          const lineColor = lineCategoryColors.get(category.name) || "blue";
          return (
            <LegendItem
              key={`item-${index}`}
              name={category.name}
...
const ChartLegend = (
  ...
  const paddingRight =
    (legendPosition === "right" || legendPosition === undefined) && lineYAxisWidth ? lineYAxisWidth - 8 : 52;
        ...
        <div className={cx("space-y-1 px-4 py-2")}>
          {filteredPayload.map(({ value, category, barColor, lineColor, chartType }, index) => (
            <div key={`id-${index}`} className="flex items-center justify-between space-x-8">
              ...
              >
                {chartType === "bar" ? barValueFormatter(value) : lineValueFormatter(value)}
              </p>
            </div>
          ))}
        </div>
...
const ComboChart = React.forwardRef<HTMLDivElement, ComboChartProps>((props, forwardedRef) => {
  const defaultSeries = {
    categories: [],
  ...
  const paddingValue =
    (!showXAxis && !mergedBarSeries.showYAxis && enableBiaxial && !mergedLineSeries.showYAxis) ||
    (startEndOnly && !mergedBarSeries.showYAxis && enableBiaxial && !mergedLineSeries.showYAxis)
      ? 0
      : 20;
  ...
  return (
    <div ref={forwardedRef} className={cx("h-80 w-full", className)} tremor-id="tremor-raw" {...other}>
      <ResponsiveContainer>
        ...
        >
          {showGridLines ? (
            <CartesianGrid
              className={cx("stroke-neutral-200 stroke-1 dark:stroke-neutral-800")}
              horizontal={true}
              vertical={false}
            />
          ) : null}
          ...
          {mergedBarSeries.categories.map((category) => (
            <Bar
              yAxisId={enableBiaxial ? "left" : undefined}
              className={cx(
                getColorClassName(barCategoryColors.get(category) as AvailableChartColorsKeys, "fill"),
          ...
          {mergedLineSeries.categories.map((category) => (
            <Line
              yAxisId={enableBiaxial ? "right" : undefined}
              className={cx(
                getColorClassName(lineCategoryColors.get(category) as AvailableChartColorsKeys, "stroke"),
                ...
                return (
                  <Dot
                    className={cx(
                      ...
                      onValueChange ? "cursor-pointer" : "",
                      getColorClassName(lineCategoryColors.get(dataKey) as AvailableChartColorsKeys, "fill"),
                    )}
                    cx={cxCoord}
                    cy={cyCoord}
                    r={5}
