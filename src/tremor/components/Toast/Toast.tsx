...
const Toast = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Root>, ToastProps>(
  ({ className, variant, title, description, action, disableDismiss = false, ...props }: ToastProps, forwardedRef) => {
    ...
    switch (variant) {
      case "success":
        Icon = (
          <RiCheckboxCircleFill className="size-5 shrink-0 text-emerald-600 dark:text-emerald-500" aria-hidden="true" />
        );
        break;
      case "warning":
        Icon = <RiErrorWarningFill className="size-5 shrink-0 text-amber-500 dark:text-amber-500" aria-hidden="true" />;
        break;
      case "error":
        Icon = <RiCloseCircleFill className="size-5 shrink-0 text-red-600 dark:text-red-500" aria-hidden="true" />;
        break;
      case "loading":
        Icon = (
          <RiLoader2Fill
            className="size-5 shrink-0 animate-spin text-neutral-600 dark:text-neutral-500"
            aria-hidden="true"
          />
        );
        break;
      default:
        Icon = <RiInformationFill className="size-5 shrink-0 text-blue-500 dark:text-blue-500" aria-hidden="true" />;
        break;
    }
      ...
      >
        <div
          className={cx(
            ...
            // border
            !disableDismiss || action ? "border-r border-neutral-200 dark:border-neutral-800" : "",
          )}
        >
          {Icon}
          <div className="flex flex-col gap-1">
            {title && (
              <ToastPrimitives.Title className="text-sm font-semibold text-neutral-900 dark:text-neutral-50">
                {title}
              </ToastPrimitives.Title>
            )}
            {description && (
              <ToastPrimitives.Description className="text-sm text-neutral-600 dark:text-neutral-400">
                {description}
