...
>(({ className, shortcut, hint, children, ...props }, forwardedRef) => (
  ...
  >
    {children}
    {hint && <span className={cx("ml-auto pl-2 text-sm text-neutral-400 dark:text-neutral-600")}>{hint}</span>}
    {shortcut && <span className={cx("ml-auto pl-2 text-sm text-neutral-400 dark:text-neutral-600")}>{shortcut}</span>}
  </DropdownMenuPrimitives.Item>
));
  ...
  >
    <span className="absolute left-2 flex size-4 items-center justify-center">
      <DropdownMenuPrimitives.ItemIndicator>
        <RiCheckLine aria-hidden="true" className="size-full shrink-0 text-neutral-800 dark:text-neutral-200" />
      </DropdownMenuPrimitives.ItemIndicator>
    </span>
    {children}
    {hint && <span className={cx("ml-auto text-sm font-normal text-neutral-400 dark:text-neutral-600")}>{hint}</span>}
    {shortcut && (
      <span
        className={cx(
...
>(({ className, ...props }, forwardedRef) => (
  <DropdownMenuPrimitives.Separator
    ref={forwardedRef}
    className={cx("-mx-1 my-1 h-px border-t border-neutral-200 dark:border-neutral-800", className)}
    {...props}
  />
));
