...
const SparkAreaChart = React.forwardRef<HTMLDivElement, SparkAreaChartProps>((props, forwardedRef) => {
  ...
  return (
    <div ref={forwardedRef} className={cx("h-12 w-28", className)} tremor-id="tremor-raw" {...other}>
      <ResponsiveContainer>
        <RechartsAreaChart
          data={data}
          margin={{
    ...
    <div ref={forwardedRef} className={cx("h-12 w-28", className)} tremor-id="tremor-raw" {...other}>
      <ResponsiveContainer>
        ...
        >
          ...
          {categories.map((category) => (
            <Bar
              className={cx(getColorClassName(categoryColors.get(category) as AvailableChartColorsKeys, "fill"))}
              key={category}
              name={category}
              type="linear"
              dataKey={category}
